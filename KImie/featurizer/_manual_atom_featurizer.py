...
from KImie.featurizer._atom_featurizer import (
    FixedSizeAtomFeaturizer,
...
from KImie.utils.mol import ATOMIC_SYMBOL_NUMBERS, ATOM_PROPS
...
class Atom_ConnectedAtoms_Featurizer(FixedSizeAtomFeaturizer):
    ...
    def as_dict(self):
        d = super(FixedSizeAtomFeaturizer, self).as_dict()
    ...
    def describe_features(self):
        if self._feature_descriptions is None:
            n = str(self)
            return ["{}: {}".format(n, v) for v in self.atoms]
...
def atom_symbol_one_hot_from_set_of_mols_featurizer(
    ...
    from KImie.mol.molecule import Molecule
...
__all__ = [
    ...
    "Atom_PaulingElectronegativity_Featurizer",
    "atom_PaulingElectronegativity_featurizer",
]
