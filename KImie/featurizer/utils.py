...
def merge_atom_featurizer_data(atom_featurizer:Prefeaturizer,return_split_indices:bool=False):
    """
...
def reduce_features(data,feature_names=None,min_rel_content:float=0.0,max_rel_content:float=None):
    """
    ...
    for d in range(data.shape[1]):
        uniques,indices,counts = np.unique(data[:,d],axis=0,return_counts=True,return_index=True)
