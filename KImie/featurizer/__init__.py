...
def generate_featurizer_df(featurizer_module) -> pd.DataFrame:
    """
    Generates a dataframe with the information of the featurizer in the module
    :param featurizer_module: a module with a featurizer, needs an get_available_features() function, which returns a  Dict[str,Featurizer]
    :return: a dataframe with the information of the featurizer
    """

    if not hasattr(featurizer_module, "get_available_featurizer"):
        raise ValueError(
            "The featurizer module does not have 'get_available_featurizer' function"
        )
    all_featurizer = []
    ...
    new_infos = pd.DataFrame(raw_infos, index=all_featurizer_names)
    return new_infos


def get_molecule_featurizer_info() -> pd.DataFrame:
    """
    Generates a dataframe with the information of registered molecule featurizers
    :return: pd.DataFrame with the information of registered molecule featurizers
    """
    from KImie.featurizer import molecule_featurizer
...
def get_atom_featurizer_info():
    """
    Generates a dataframe with the information of registered atom featurizers
    :return: pd.DataFrame with the information of registered atom featurizers
    """
    from KImie.featurizer import atom_featurizer

    df = generate_featurizer_df(atom_featurizer)
    return df


def get_featurizer_folder() -> str:
    """
    Returns the folder where the featurizer data is stored
    :return: str
    """
...
def reset_normalization_data() -> None:
    # resets the user featurizer data by the repository data
    KIMIE_LOGGER.debug("resets normalization excel data")
    import shutil

    norm_file = os.path.join(get_featurizer_folder(), "featurizer_norm.xlsx")
    shutil.copyfile(
        os.path.join(os.path.dirname(__file__), "featurizer_norm.xlsx"), norm_file
    )


def load_normalization_excel_data() -> pd.DataFrame:
    # read the user provides normalization data to dataframe (if not provides uses the repository data)
    KIMIE_LOGGER.debug("load normalization excel data")
...
def recalculate_normalization_data():
    ...
    KIMIE_LOGGER.debug("recalculate normalization data")
            ...
            for norm_name, norm_data in dimdata.items():
                ...
                for data_range, fit_params in norm_data.items():
                    if best_range is None:
                        ...
                        if (
                            np.isnan(best_range["R2"])
                            or fit_params["R2"] < best_range["R2"]
                        ):
                            best_range = fit_params
...
def load_normalization_dict_data() -> Dict[str, List[Dict[str, Dict[str, Any]]]]:
    KIMIE_LOGGER.debug("load normalization data")
    ...
    if not os.path.exists(norm_file):
        ...
        if os.path.exists(xlsx_path) and os.path.getmtime(
            norm_file
        ) <= os.path.getmtime(xlsx_path):
            recalculate_normalization_data()
...
def get_normalization_data() -> Dict[str, List[Dict[str, Dict[str, Any]]]]:
    """
