...
from KImie.featurizer._atom_featurizer import FixedSizeAtomFeaturizer, _AtomFeaturizer
from KImie.featurizer._molecule_featurizer import (
    FixedSizeMoleculeFeaturizer,
    _MoleculeFeaturizer,
)
from KImie.featurizer.featurizer import Featurizer, FixedSizeFeaturizer

valid_chars = "-_.(), %s%s" % (string.ascii_letters, string.digits)


def molecule_fixed_size_featuizer_handler(
    featurizer: FixedSizeMoleculeFeaturizer,
    dataloader: MolDataLoader,
    data_path,
    n_split=100_000,
):
    ...
    files = [
        os.path.join(data_path, f"feats_{range_start[i]+1}_{e}.npy")
        for i, e in enumerate(range_start[1:])
    ]
    file_sizes = [e - range_start[i] for i, e in enumerate(range_start[1:])]
    ...
    def turnover(current_start_index):
        ...
        current_array = (
            np.zeros((file_sizes[indices[current_start_index]], featurizer_length))
            * np.nan
        ).astype(featurizer_dtype)
    ...
    dataloader.close()
    bn = (
        os.path.basename(os.path.dirname(data_path)) + "/" + os.path.basename(data_path)
    )
    for i, mol in tqdm(
        enumerate(dataloader), desc=f"featzurize mols ({bn})", total=lenmols
    ):
        if i < start_index:
            continue
        try:
...
def atom_fixed_size_featuizer_handler(
    featurizer: FixedSizeAtomFeaturizer,
    dataloader: MolDataLoader,
    data_path,
    n_split=10_000,
):
    ...
    for i, mol in tqdm(
        ...
        if ri >= ri_max:
            ...
            current_array = (np.zeros((n_atoms, featurizer_length)) * np.nan).astype(
                featurizer_dtype
            )
...
class Prefeaturizer:
    def __init__(
        ...
        featurizer_handler=None,
        iterator=None,
        basedir=os.path.join(get_user_folder(), "featurized_data", "feats_raw"),
    ):
    ...
    def _gen_path(self):
        self._data_path = os.path.join(
            self.basedir, self.dataset_name, self.featurizer_name
        )
    ...
    def feature_names(self):
        return self.featurizer.describe_features()
    ...
    def prefeaturize(self, recalculate=False, ignore_working=False):
        ...
        if self.is_working() and not ignore_working:
            KIMIE_LOGGER.error(
                f"prefeaturizer '{self.dataset_name}.{self.featurizer_name}' already working (info)"
            )
            return False

        if self.is_done() and not recalculate:
            KIMIE_LOGGER.info(
                f"prefeaturizer '{self.dataset_name}.{self.featurizer_name}' already done"
            )
            return True

        if self.is_locked() and not ignore_working:
            KIMIE_LOGGER.error(
                f"prefeaturizer '{self.dataset_name}.{self.featurizer_name}' already working (lockfile)"
            )
            return False
    ...
    def prefeaturize_if_needed(self):
        if not self.is_done():
            succ = self.prefeaturize()
            if not succ:
                raise PrefeaturizerException(
                    f"prefeaturizer '{self.dataset_name}.{self.featurizer_name}' failed"
                )

    def __iter__(self):
        self.prefeaturize_if_needed()
        return self.iterator(self.data_path, self.featurizer)
