...
def gen_ecdf(data):
    ...
    if ecdf.shape[0] == 1:
        ecdf = np.array([0.0, 1.0])
        values = np.array([values[0] - 1.0, values[0]])
...
class ECDFNormalization(Normalization):
    ...
    def fit(self, x):
        ...
        for d in range(self._dimensions):
            ...
            x_use = np.array(
                [values[0], values[-1]] + [values[-1]] * (self._max_resolution - 2)
            )
            ...
            for i in range(2, max(3, self._max_resolution)):
                if np.all(ignore_mask):
                    ...
                    break
                error1 = (
                    np.interp(values[~ignore_mask], x_use, y_use) - ecdf[~ignore_mask]
                ) ** 2
                error2 = (
                    (np.interp(ecdf[~ignore_mask], y_use, x_use) - values[~ignore_mask])
                    / data_dist
                ) ** 2
            ...
            if np.all(ignore_mask):
                ...
                if np.isnan(me):
                    raise ValueError(
                        f"ECDF normalization failed ({data_dist},{error1},{error2},{error},{me})"
                    )
            KIMIE_LOGGER.info(
                f"normalization of feature {d} fitted with {x_use.shape[0]} points and an error of {me}"
            )
            self._x_use[d] = x_use
            self._y_use[d] = y_use
    ...
    def normalize(self, x):
        ...
        if x.shape[1] != self._dimensions:
            raise ValueError(
                f"Dimension mismatch (got {x.shape[1]}, expected {self._dimensions})"
            )
        out = np.zeros_like(x)
        for d in range(self._dimensions):
