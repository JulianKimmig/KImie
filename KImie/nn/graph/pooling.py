...
class PoolWeightedSum(PoolingBase):
    def __init__(self, n_in_feats=None,n_out_feats=1, normalize=True, bias=True):
        ...
        if self._normalize:
            ...
            self.weighting_of_nodes = nn.Linear(n_in_feats, self._n_out_feats, bias=self._bias)
    ...
    def forward(self, feats, batch):
        ...
        weight_feats = torch.einsum ('io, if -> iof', weights, feats) # dims = nodes,out_feats,last_gcn_feats
        weight_feats = weight_feats.flatten(start_dim=1, end_dim=- 1)  # dims = nodes,out_feats*last_gcn_feats
        summed_nodes = scatter_add(weight_feats, batch, dim=0) # dims = out_feats*last_gcn_feats
        return summed_nodes
